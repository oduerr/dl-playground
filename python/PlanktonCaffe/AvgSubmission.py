...
fout = open('/Users/oli/Proj_Large_Data/kaggle_plankton/submission_6_8_join.txt', 'w')
w = csv.writer(fout)

fst =  fc1.next();fc2.next();fc3.next();fc4.next();
w.writerow(fst)
...
for row1 in fc1:
    ...
    for c in range(1, len(row1)):
        newLine.append(str((float(row1[c]) + float(row2[c]) + float(row3[c]) + float(row4[c]))/4.0))
    w.writerow(newLine);
fout.close()
