...
system("~/caffe/caffe/tools/extra/parse_log.sh log_lenet_enhanced_with_lbh.txt")
train.do = read.table('log_lenet_enhanced_with_lbh.txt.train', header = TRUE, comment.char = 'H')
test.do = read.table('log_lenet_enhanced_with_lbh.txt.txt.test', header = TRUE, comment.char = 'H')
...
gg <- gg + geom_hline(y=1.79, size = 0.15) + ylim(0,0.2)
